...
class DependencyContainer(private val application: Application) {
    ...
    val serviceRegistry: ServiceRegistry by lazy {
        ServiceRegistry(httpClient).apply {
            // Services Kotlin hardcodés (core services)
            ...
            // Charger les services YAML automatiquement
            try {
                loadYamlServices()
                println("✅ YAML services loaded successfully")
            } catch (e: Exception) {
                println("⚠️  Failed to load YAML services: ${e.message}")
                e.printStackTrace()
            }
        }
    }
